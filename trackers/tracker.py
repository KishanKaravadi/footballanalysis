...
class Tracker:
    ...
    def interpolate_ball_positions(self, ball_positions):
        ...
        # Edge case in which first couple frames are missing, it backfills it
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            # Convert Goalkeeper to player object in class_id

            for object_ind, class_id in enumerate(detection_supervision.class_id):
                if cls_names[class_id] == 'goalkeeper':
                    detection_supervision.class_id[object_ind] = cls_names_inv['player']

            # Convert Goalkeeper to player object in class_names

            for object_ind, class_name in enumerate(detection_supervision.data['class_name']):
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        ...
        if track_id is not None:
            ...
            cv2.putText(frame, f"{track_id}", (int(x1_text), int(
                y1_rect+15)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
